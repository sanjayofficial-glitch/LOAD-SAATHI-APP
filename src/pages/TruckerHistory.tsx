        ...
        <div className="flex justify-between items-center">
          ...
          <div className="flex items-center space-x-4">
            <Badge variant="default" className="bg-gray-100 text-gray-700 px-2 py-1 rounded">
              {user?.user_type === 'trucker' ? 'Trucker' : 'Unknown'}
            </Badge>
          </div>
        </div>
      </div>
            ...
            <SelectTrigger asChild>
              ...
              <SelectOverlay asChild className="absolute left-0 w-full">
                {[
                  ...
                  { value: 'incoming_request', label: 'Incoming Request' },
                  { value: 'outgoing_shipment_request', label: 'Outgoing Shipment Request' },
                  { value: 'completed_shipment', label: 'Completed Shipment' },
                ].map((option) => (
...
function renderActivityIcon(type: string) {
  switch (type) {
    ...
    case 'outgoing_shipment_request':
      return <Package className="h-6 w-6 text-purple-600" />;
    case 'completed_shipment':
